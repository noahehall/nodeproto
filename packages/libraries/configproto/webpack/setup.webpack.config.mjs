...
export default function setupWebpack({
  ...
  const r = (f = throwMsg('pathOnDisk', 'error in webpack/setup.resolve'), c = context) => path.resolve(c, f);

  const getFile = (
    f = throwMsg('pathOnDisk', 'error in webpack/setup.getFile: '),
  ) => readFileSync(r(f, context), 'utf8');

  const pkgJson = JSON.parse(getFile(pkgJsonPath));
  const pathDist = r(pkgJson.config.PATH_DIST);
  const pathSrc = r(pkgJson.config.PATH_SRC);
  const mode = NODE_ENV || pkgJson.config.NODE_ENV;
  const ifProd = isProd || mode === 'production';
  const ifDev = isDev || !ifProd;
