...
const fileShouldCopy = sourcepath => {
  console.info('\n\n checking if we should copy file', sourcepath);
...
const fileCopy = (sourcepath, outdir) => {
  console.info('\n\n copying file into outdir', sourcepath, outdir);
  try {
      ...
      const sourcedirs = fs.readdirSync(indir, { encoding: 'utf8', withFileTypes: true }) ?? [];
        ...
        if (!dirEnt.name.includes('.') && recurse) filesToCopy([
          ...
          if (endingWith.test(dirEnt.name)) {
            ...
            console.info('n\n found file to copy', { sourcepath, outpath})
...
export default function popCopy (config) {
  ...
  return {
    ...
    setup (build) {
       ...
       *          { options: [{
       *              indir: '../../app',
       *              outdir: 'outdir', // 'mirror'|'./path/relative/to/esbuild.config.outdir'
